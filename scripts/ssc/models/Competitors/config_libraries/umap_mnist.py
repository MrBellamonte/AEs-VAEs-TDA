...
umap_mnist3d_euler_multi = [ConfigGrid_Competitors(
    model_class = [UMAP],
    model_kwargs=dict(n_components = [3],test_eval=[False],n_neighbors = np.linspace(10,500,15).astype(int).tolist(), min_dist = np.linspace(0.1,1,10).tolist()),
    dataset=[MNIST_offline()],
    sampling_kwargs=[dict(root_path='/cluster/home/schsimo/MT/AEs-VAEs-TDA', n_samples = 10000)],
    eval=[ConfigEval(
        active=True,
        ...
        k_step=4)],
    uid = [''],
    experiment_dir='/cluster/scratch/schsimo/output/umap_mnist3d',
    seed = seed,
    verbose = True
) for seed in [1110,1909,1318]]

umap_mnist3d_euler_multi_list = list(itertools.chain(*[config_grid.configs_from_grid() for config_grid in umap_mnist3d_euler_multi]))
...
tsne_mnist3d_euler_multi = [ConfigGrid_Competitors(
    model_class = [tSNE],
    model_kwargs=dict(n_components = [3],n_jobs = [1],perplexity = np.linspace(5,50,10).astype(int).tolist()),
    dataset=[MNIST_offline()],
    sampling_kwargs=[dict(root_path='/cluster/home/schsimo/MT/AEs-VAEs-TDA', n_samples = 10000)],
    eval=[ConfigEval(
        active=True,
        ...
        k_step=4)],
    uid = [''],
    experiment_dir='/cluster/scratch/schsimo/output/mnist_tsne3d',
    seed = seed,
    verbose = True
) for seed in [1110,1909,1318]]

tsne_mnist3d_euler_multi_list = list(itertools.chain(*[config_grid.configs_from_grid() for config_grid in tsne_mnist3d_euler_multi]))
