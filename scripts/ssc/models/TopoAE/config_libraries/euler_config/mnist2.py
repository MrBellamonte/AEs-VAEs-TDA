...
from src.models.autoencoder.autoencoders import Autoencoder_MLP_topoae, ConvAE_MNIST, DeepAE_MNIST
...
mnist_1_deep = [ConfigGrid_TopoAE(
    learning_rate=[1/10,1/100,1/1000],
    batch_size=[64, 128, 256],
    n_epochs=[1000],
    weight_decay=[1e-6],
    ...
    uid=[''],
    method_args=[None],
    experiment_dir='/cluster/scratch/schsimo/output/mnist_topoae_1_deepae',
    seed=seed,
    device='cpu',
    num_threads=1,
    verbose=False
...
mnist_1_deep_list = list(itertools.chain(*[config_grid.configs_from_grid() for config_grid in mnist_1_deep]))



mnist_2_deep = [ConfigGrid_TopoAE(
    learning_rate=[1/10,1/100,1/1000],
    batch_size=[64, 128, 256],
    n_epochs=[1000],
    weight_decay=[1e-6],
    early_stopping=[32],
    rec_loss_weight=[1],
    top_loss_weight=[i for i in np.logspace(-8, -6, num=3, base=2.0)],
    toposig_kwargs=[dict(match_edges='symmetric')],
    model_class=[DeepAE_MNIST],
    model_kwargs=[dict()],
    dataset=[MNIST_offline()],
    ...
    uid=[''],
    method_args=[None],
    experiment_dir='/cluster/scratch/schsimo/output/mnist_topoae_2_deepae',
    seed=seed,
    device='cpu',
    num_threads=1,
    verbose=False
...
mnist_2_deep_list = list(itertools.chain(*[config_grid.configs_from_grid() for config_grid in mnist_2_deep]))
