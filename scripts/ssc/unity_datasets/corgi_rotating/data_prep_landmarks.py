...
root_path_save ='/Users/simons/PycharmProjects/MT-VAEs-TDA/src/datasets/simulated/corgi_rotation_{}_l'.format(version)
...
deg_landmarks = np.linspace(1,360,2*bs,dtype=int)
ind_l = np.in1d(positions, deg_landmarks).nonzero()[0]
ind_nl = np.array(list(set(ind_all)-set(ind_l)))

positions = torch.Tensor(positions)

ind_train_nl = ind_nl[:210]
ind_train = np.append(ind_l,ind_train_nl)
ind_eval = ind_nl[210:270]
ind_test = ind_nl[270:]
