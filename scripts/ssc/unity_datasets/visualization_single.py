...
if __name__ == "__main__":
    ...
    poshr = (position[:, 0:2].numpy() == (-5, 4.5)).all(axis=1).nonzero()
    poshl = (position[:, 0:2].numpy() == (-5, -5)).all(axis=1).nonzero()
    posc = (position[:, 0:2].numpy() == (0, 0)).all(axis=1).nonzero()
    posvl = (position[:, 0:2].numpy() == (4.5, -5)).all(axis=1).nonzero()
    posvr = (position[:, 0:2].numpy() == (4.5, 4.5)).all(axis=1).nonzero()
