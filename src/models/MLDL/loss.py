...
class MLDL_Loss(object):
    ...
    def DistanceLoss(self, data, latent, dis_data, dis_latent, kNN_data, kNN_latent):
        ...
        if 'MNIST' in self.args['DATASET']:
            ...
            Error2 = (0-D2_1)/1
        loss_push_away = torch.norm(Error2[Error2 > -1*self.args['RegularB']])/torch.sum(
            kNN_data == False)

        # The gradual changing of weight for Loss_push-away
        if self.epoch > self.args['GradualChanging'][0]:
            self.push_away = max(0.80-(self.epoch-self.args['GradualChanging'][0])/(
                        self.args['GradualChanging'][1]-self.args['GradualChanging'][0])*0.80, 0)
        else:
            self.push_away = 0.80
    ...
    def MorphicLossItem(self, data, latent):
        ...
        if 'MNIST' in self.args['DATASET'] or (
                'Spheres' in self.args['DATASET'] and self.args['Mode'] == 'ML-AE'):
        ...
        loss_iso, loss_push_away = self.DistanceLoss(data, latent, dis_data, dis_latent, kNN_data,
                                                     kNN_latent)
    ...
    def CalLosses(self, train_info):
        ...
        loss_distance, loss_ang, loss_mutex = self.MorphicLossItem(train_info[0],
                                                                   train_info[self.latent_index])
